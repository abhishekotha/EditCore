export const findingCurserLocation = ({ data, currentPosition, arraowDirection, }) => {
    let lineIndex = 0;
    let sum = 0;
...
export const handleCharacterInput = ({ key, curserPoints, data }) => {
...
export const handleEnter = ({ data, curserPoints }) => {
...
export const handleBackspace = ({ data, curserPoints, removeLength = 1 }) => {
    let newPoints = curserPoints;
    const updated = [...data];
    let removedData = "";
    if (newPoints.x > 0) {
...
export const handleRemoveText = ({ selection, divRef, data }) => {
    if (!divRef)
        return null;
    ...
    children[startLineIndex].childNodes.forEach((item) => {
        ...
        if (item.contains(startContainer))
            actualStartIndex = count + startOffset;
        count += ((_a = item.textContent) === null || _a === void 0 ? void 0 : _a.length) || 0;
    });
    let actualEndIndex = 0;
    count = 0;
    children[endLineIndex].childNodes.forEach((item) => {
        var _a;
        if (item.contains(endContainer))
            actualEndIndex = count + endOffset;
        count += ((_a = item.textContent) === null || _a === void 0 ? void 0 : _a.length) || 0;
    });
    const removedText = startLineIndex === endLineIndex
...
export const handelTabOperation = ({ data, curserPoints }) => {
    let newPoints = curserPoints;
    const tabSpaces = "    ";
    const updated = [...data];
...
export const handleRemoveByCoordinates = ({ data, start, end }) => {
    const updatedData = [...data];
    let removedData = "";
    if (start.y === end.y) {
...
export const handlePasteText = ({ text, curserPoints, data }) => {
...
export const handleUndoAction = ({ undoAction, data }) => {
...
export const handleRndoAction = ({ redoAction, data }) => {
...
export const setCursorInline = (divRef, curserPoints) => {
    var _a;
    ...
    Array.from(lineElement.childNodes).some((item) => {
        var _a, _b;
        const textLength = ((_a = item.textContent) === null || _a === void 0 ? void 0 : _a.length) || 0;
        temp += textLength;
        if (temp >= curserPoints.x) {
            textNode = item.firstChild || item;
            offset = curserPoints.x - (temp - textLength);
            offset = Math.max(0, Math.min(offset, ((_b = textNode.textContent) === null || _b === void 0 ? void 0 : _b.length) || 0));
            return true;
        }
        return false;
    });
    if (!textNode) {
        ...
        offset = Math.min(curserPoints.x, ((_a = textNode === null || textNode === void 0 ? void 0 : textNode.textContent) === null || _a === void 0 ? void 0 : _a.length) || 0);
    }
    if (!textNode)
        return;
...
export const getSelectionCoordinates = ({ divRef }) => {
    var _a;
    const selection = window.getSelection();
    const div = divRef;
    if (selection && div) {
        ...
        for (let i = 0; i < allNodeChildren.length; i++) {
            ...
            if (temp.contains(startnode)) {
                ...
                startPoint += ((_a = temp.textContent) === null || _a === void 0 ? void 0 : _a.length) || 0;
            }
        }
